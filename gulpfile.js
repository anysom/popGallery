...
var settings = {
  ...
  baseDir:            'www/',
  scriptsDir:         'js/',
  scriptName:         'PopGallery.js',
  mainSassFile:       'PopGallery.scss',
  stylesDir:          'css/'
};
...
gulp.task('sass', function () {
  gulp.src(settings.stylesDir+settings.mainSassFile)
    ...
    .on('error', handleError)
    .pipe(minifyCSS())
    .pipe(rename('PopGallery.min.css'))
    .pipe(gulp.dest(settings.stylesDir))
    .pipe(reload({stream: true}));
});

gulp.task('distribute', function () {
  gulp.src([settings.scriptsDir+'PopGallery.min.js', settings.stylesDir+'PopGallery.min.css'])
    .pipe(copy('dist', {
      prefix: 2
    }));
});
